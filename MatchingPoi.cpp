...
MatchingPoints::MatchingPoints (Mat image1, Mat image2) {
	Matcher goodMatcher;
	goodMatcher.setConfidenceLevel(0.99);
	goodMatcher.setMinDistanceToEpipolar(3.0);
	goodMatcher.setRatio(0.8f);
    cv::Ptr<SURF> pfd = SURF::create(10);
...
void MatchingPoints::displayFull(Mat image1, Mat image2) 
{
	...
	imwrite("/Users/nyan/Downloads/Output.jpg", imageMatches);
}
